/* 02_stored_procedures.sql
   SPs para aplicar eventos de Facturación hacia CxC
*/

USE CxC;
GO

-- Recalcula el estado basado en saldo
...
-- Aplica una FACTURA (crea Documento + Movimiento CARGO)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            @Total = f.Total,
            @Fecha = CAST(f.Fecha AS DATE),
            @Venc = CAST(f.FechaVencimiento AS DATE),
            @EstadoFactura = f.Estado
        FROM FacturacionCarol.dbo.Factura f
        WHERE f.FacturaId = @FacturaId;

        IF @ClienteId IS NULL
            THROW 50001, 'Factura no encontrada en FacturacionCarol.', 1;
        ...
        FROM dbo.CxC_Documento
        WHERE TipoDocumento='FAC' AND ReferenciaExterna=@Ref;

        IF @DocumentoId IS NULL
        BEGIN
            INSERT INTO dbo.CxC_Documento
                (ClienteId, TipoDocumento, ReferenciaExterna, FechaEmision, FechaVencimiento, MontoOriginal, SaldoActual, Estado)
            VALUES
                (@ClienteId, 'FAC', @Ref, @Fecha, @Venc, @Total, @Total, 'ABIERTO');

            SET @DocumentoId = SCOPE_IDENTITY();

            INSERT INTO dbo.CxC_Movimiento
                (DocumentoId, TipoMovimiento, Monto, ReferenciaExterna, Descripcion)
            VALUES
                (@DocumentoId, 'CARGO', @Total, @Ref, 'Cargo por factura');
        END

        -- Si viene anulada desde origen, reflejarlo
        ...
        INSERT INTO dbo.CxC_IntegracionAplicada (TipoEvento, EntidadId)
        VALUES ('FACTURA_CREADA', @FacturaId);
...
-- Aplica un PAGO (Movimiento PAGO + baja saldo)
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @FacturaId BIGINT,
                @Monto DECIMAL(18,2),
                @FechaPago DATETIME2,
                @RefPago VARCHAR(50) = CONCAT('PAGO-', @PagoId),
                @RefFac VARCHAR(50);
        ...
        FROM FacturacionCarol.dbo.Pago p
        WHERE p.PagoId = @PagoId;

        IF @FacturaId IS NULL
            THROW 50002, 'Pago no encontrado en FacturacionCarol.', 1;
        ...
        SELECT @DocumentoId = DocumentoId
        FROM dbo.CxC_Documento
        WHERE TipoDocumento='FAC' AND ReferenciaExterna=@RefFac;

        -- Si la factura no existe aún en CxC, primero la aplicamos
        IF @DocumentoId IS NULL
        BEGIN
        ...
        -- Si está anulada, no aplicar pago
        ...
        INSERT INTO dbo.CxC_Movimiento
            (DocumentoId, TipoMovimiento, Monto, Fecha, ReferenciaExterna, Descripcion)
        VALUES
            (@DocumentoId, 'PAGO', @Monto, @FechaPago, @RefPago, 'Pago aplicado desde Facturación');
...
-- Aplica una NOTA DE CRÉDITO (Movimiento ABONO + baja saldo)
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @FacturaId BIGINT,
                @Monto DECIMAL(18,2),
                @Fecha DATETIME2,
                @RefNC VARCHAR(50) = CONCAT('NC-', @NotaCreditoId),
                @RefFac VARCHAR(50);
        ...
        SELECT
            ...
            @Monto = n.Monto,
            @Fecha = n.Fecha
        FROM FacturacionCarol.dbo.NotaCredito n
        WHERE n.NotaCreditoId = @NotaCreditoId;

        IF @FacturaId IS NULL
            THROW 50004, 'Nota de crédito no encontrada en FacturacionCarol.', 1;
        ...
        INSERT INTO dbo.CxC_Movimiento
            (DocumentoId, TipoMovimiento, Monto, Fecha, ReferenciaExterna, Descripcion)
        VALUES
            (@DocumentoId, 'ABONO', @Monto, @Fecha, @RefNC, 'Nota de crédito aplicada desde Facturación');
        ...
        INSERT INTO dbo.CxC_IntegracionAplicada (TipoEvento, EntidadId)
        VALUES ('NOTA_CREDITO_CREADA', @NotaCreditoId);
...
-- Anula una FACTURA en CxC (Estado ANULADO + Saldo 0 + Movimiento ANULACION)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @DocumentoId = DocumentoId, @Saldo = SaldoActual
        FROM dbo.CxC_Documento
        WHERE TipoDocumento='FAC' AND ReferenciaExterna=@RefFac;
        ...
        BEGIN
            INSERT INTO dbo.CxC_IntegracionAplicada (TipoEvento, EntidadId)
            VALUES ('FACTURA_ANULADA', @FacturaId);
        ...
        BEGIN
            INSERT INTO dbo.CxC_Movimiento
                (DocumentoId, TipoMovimiento, Monto, ReferenciaExterna, Descripcion)
            VALUES
                (@DocumentoId, 'ANULACION', @Saldo, @RefFac, 'Anulación de factura: saldo cancelado');
        END

        UPDATE dbo.CxC_Documento
        SET Estado='ANULADO', SaldoActual=0
        WHERE DocumentoId=@DocumentoId;
