...
BEGIN
    ...
    CREATE UNIQUE INDEX UX_CxC_Documento_Ref
    ON dbo.CxC_Documento (TipoDocumento, ReferenciaExterna);

    CREATE INDEX IX_CxC_Documento_Cliente
    ON dbo.CxC_Documento (ClienteId, Estado);
END
GO
...
BEGIN
    CREATE TABLE dbo.CxC_Movimiento (
        ...
        CONSTRAINT FK_CxC_Mov_Doc FOREIGN KEY (DocumentoId)
            REFERENCES dbo.CxC_Documento(DocumentoId),
    ...
    CREATE INDEX IX_CxC_Mov_DocFecha
    ON dbo.CxC_Movimiento (DocumentoId, Fecha);
END
GO


-- 3) Idempotencia (anti duplicados cuando el Job reintenta)
